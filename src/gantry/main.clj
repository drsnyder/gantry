...
(defn -main [& args]
  (with-command-line args
      ...
      [[hosts          H  "The remote hosts" ""]
       [port           p  "The ssh port of the remote hosts" 22]
       ...
       [arg-settings  s  "Add arguments to be passed down to the actions" nil]
       actions]
    (do 
      
      (handler-case :type
                    (let [base (resolve-targets config-file (re-split #"," hosts))
                          config (set-args base (merge-settings {:port port} arg-settings))]
